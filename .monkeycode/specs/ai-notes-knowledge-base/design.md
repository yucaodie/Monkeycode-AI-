...
| 路径 | 页面 | 说明 |
|------|------|------|
| `/notes` | 笔记模块 | 三栏布局：笔记树（笔记本 > 笔记）/ 笔记页编辑器 / 笔记页列表（OneNote 风格） |
...
├── Pages/
│   ├── NotesPage.tsx (默认首页)
│   │   ├── NoteTree.tsx (左侧栏 - 笔记树：笔记本 > 笔记)
│   │   ├── TipTapEditor.tsx (中间栏 - 笔记页富文本编辑器)
│   │   └── PageList.tsx (右侧栏 - 选中笔记下的笔记页列表)
...
NoteTree 是左侧导航树组件，仅展示笔记本 > 笔记两级层级（笔记页不在树中，在中间 PageList 中展示）。

**视觉层级**：
- 笔记本节点：一级节点，粗体，带笔记本图标，点击展开/折叠
- 笔记节点：二级节点，普通字体，带笔记图标，缩进 12px，点击选中后在中间栏展示笔记页

**交互行为**：
- 展开/折叠：点击笔记本节点左侧的箭头图标切换展开状态
- 选中笔记：点击笔记节点，高亮并触发中间栏加载该笔记下的笔记页列表
- 右键菜单：
  - 笔记本：新建笔记、重命名、删除
  - 笔记：重命名、删除
- 拖拽排序：笔记可在笔记本之间拖拽移动
...
- Tree API 简化：`GET /api/notes/tree` 仅返回笔记本 + 笔记（不含笔记页）

#### PageList 组件规格

PageList 是右侧栏组件，展示当前选中笔记下的所有笔记页。

**视觉样式**：
- 顶部工具栏：笔记名称 + "新建笔记页"按钮
- 笔记页列表：垂直排列于工具栏下方，每个笔记页显示标题，支持点击选中，选中项高亮
- 右键菜单：重命名、删除

**交互行为**：
- 点击笔记页：选中并加载内容到中间编辑器
- 新建：点击工具栏"新建笔记页"按钮，在右侧列表新增并在中间编辑器打开
- 空状态：未选中笔记时显示"请从左侧选择笔记"

**数据加载**：
- 选中笔记时：`GET /api/notes/notes/:noteId/pages`
- 新建笔记页：`POST /api/notes/notes/:noteId/pages`
...
`GET /api/notes/tree` 返回结构（仅笔记本 + 笔记，不含笔记页）：
```json
[
  {
    "id": 1,

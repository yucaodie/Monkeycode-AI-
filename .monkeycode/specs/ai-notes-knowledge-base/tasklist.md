...
- [ ] 2. 实现 AI 模型配置管理
  - [ ] 2.1 创建 ModelConfig 数据模型和 Repository
...
- [ ] 4. 检查点 - 确保 Phase 1 代码可编译且模型管理接口可调通

- [ ] 5. 实现知识库文件夹管理
  - [ ] 5.1 创建 KnowledgeFolder 和 KnowledgeFile 数据模型和 Repository
...
- [ ] 8. 检查点 - 确保知识库后端 API 完整可测

- [ ] 9. 实现笔记层级管理
  - [ ] 9.1 创建 Notebook、Note、NotePage 数据模型和 Repository
    - 在 `backend/src/models/` 中定义接口
...
- [ ] 13. 实现知识库前端页面
  - [ ] 13.1 实现 FolderList 和 FilePanel 组件
...
- [ ] 14. 实现笔记前端页面
  - [ ] 14.1 实现三栏布局和层级管理组件
    - NotebookList（左侧栏）、NoteList（中间栏）、NotePageEditor（右侧栏）
    - 支持创建/重命名/右键菜单/拖拽排序
    - 需求引用：需求 1 验收标准 1-7
  - [ ] 14.2 集成 TipTap 富文本编辑器
...
- [ ] 23. 模板输出功能迁移
  - 将现有 `backend/src/routes/output.ts` 和 `OutputService` 保留，确保 `/api/output` 路由继续可用
  - 将现有 `frontend/src/components/OutputPanel.tsx` 迁移为独立 `/output` 页面（OutputPage）
  - 扩展内容选择来源：支持从笔记页和知识库文件中选择内容进行模板整理
  - 需求引用：需求 11 验收标准 6

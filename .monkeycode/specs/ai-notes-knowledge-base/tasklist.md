...
- [ ] 14. 重建笔记前端页面（三栏布局：笔记树 + 编辑器 + 笔记页列表，模仿 OneNote）
  - [ ] 14.1 精简 NoteTree 为仅含笔记本+笔记（去除笔记页节点）
    - 笔记本节点点击展开/折叠显示其下笔记
    - 笔记节点点击选中，触发右侧栏加载笔记页列表
    - 右键菜单：笔记本(新建笔记/重命名/删除)、笔记(重命名/删除)
    - 拖拽：笔记可在笔记本间拖拽移动
    - 内联重命名：双击节点名称原地编辑
    - 数据：复用 GET /api/notes/tree（仅返回笔记本+笔记）
    - 需求引用：需求 1 验收标准 2,3,5,6,8,9
  - [ ] 14.2 新建 PageList 右侧栏组件
    - 顶部显示选中笔记名 + "新建笔记页"按钮
    - 笔记页列表：标题展示 + 选中高亮
    - 点击笔记页加载到中间编辑器
    - 右键菜单：重命名/删除
    - 数据：GET /api/notes/notes/:noteId/pages + POST 新建
    - 需求引用：需求 1 验收标准 3,4,7
  - [ ] 14.3 重建 NotesPage 三栏布局
    - 左栏：NoteTree（宽度 ~220px）
    - 中栏：TipTapEditor（flex:1）
    - 右栏：PageList（宽度 ~200px，仅选中笔记时显示）
    - 状态流：选笔记本→展开笔记→选笔记→右侧栏显示笔记页列表→选笔记页→中间编辑
    - 需求引用：需求 1 验收标准 1
  - [ ] 14.4 简化 Tree API（后端）
    - GET /api/notes/tree 去除 pages 嵌套，仅返回笔记本+笔记
    - 保留 children 字段（空数组）以维持前端解析兼容
  - [x] 14.5 集成 TipTap 富文本编辑器（已实现，保留）
  - [x] 14.6 自动保存和版本管理（已实现，保留）
